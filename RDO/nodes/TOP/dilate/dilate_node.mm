...
// Metal 셰이더 코드 (Dilate)
static const char* dilateShaderSource = R"(
#include <metal_stdlib>
using namespace metal;

kernel void applyDilate(
    ...
    uint2 gid [[thread_position_in_grid]])
{
    ...
    float4 maxColor = float4(0.0, 0.0, 0.0, 0.0);
...
DilateNode::DilateNode(Graph<Node>& graph, const ImVec2& pos, id<MTLDevice> device)
    : device_(device)
    , dilate_pipeline_(nil)
    , command_queue_(nil)
    , kernel_size_(3)
    , iterations_(1)
    , kernel_shape_(KernelShape::Rect)
    , last_input_texture_(nil)
    , last_kernel_size_(-1)
    , last_iterations_(-1)
    , last_kernel_shape_(KernelShape::Rect)
{
...
void DilateNode::InvalidateCache()
{
...
bool DilateNode::InitializeMetal()
{
    ...
    NSError* error = nil;
    NSString* shaderCode = [NSString stringWithUTF8String:dilateShaderSource];
    ...
    id<MTLFunction> dilateFunction = [library newFunctionWithName:@"applyDilate"];

    dilate_pipeline_ = [device_ newComputePipelineStateWithFunction:dilateFunction error:&error];
    if (error) {
        NSLog(@"Error creating dilate pipeline: %@", error);
        return false;
    }
...
void DilateNode::Render(Graph<Node>& graph)
{
    const float node_width = 200.0f;
    ImNodes::BeginNode(node_id_);

    ImNodes::BeginNodeTitleBar();
    ImGui::TextUnformatted("Dilate");
    ...
    if (ImGui::Combo("Shape", &shape_idx, shapes, 3)) {
        kernel_shape_ = static_cast<KernelShape>(shape_idx);
    }
    ImGui::PopItemWidth();
...
void DilateNode::RenderInspector()
{
    ImGui::Text("Dilate");
    ImGui::Separator();
    ...
    TexturePool* texture_pool)
{
    ...
    for (int iter = 0; iter < iterations_; iter++) {
        ...
        // Dilate Pass
        [encoder setComputePipelineState:dilate_pipeline_];
...
{
    return std::make_unique<DilateNode>(graph, pos, device);
}

// 자동 등록
REGISTER_NODE(Dilate, "Dilate", "TOP/Filter", NodeFamily::TOP, CreateDilateNode, "Dilate pixels (morphological operation)");
